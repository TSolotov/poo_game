...
public class LevelCompleteOverlay {
    ...
    public LevelCompleteOverlay(CircusGame circusPlaying) {
    ...
    public void draw(Graphics g) {
        ...
        metrics = g.getFontMetrics();
        g.drawString("Score actial: " + CircusGame.getScore(),
                FRAME_WIDTH / 2 - metrics.stringWidth("Score actial: " + CircusGame.getScore()) / 2,
    ...
    public void mouseReleased(MouseEvent e) {
        if (isMouseIn(menu, e)) {
            ...
            if (play.isMousePressed()) {
                if ((LevelHandler.getNumberLevel() + 1) == circusPlaying.getLevelHandler().getCantLevels()) {
                    if (circusPlaying.getUsername().isEmpty())
                        return;
                    try (FileWriter writer = new FileWriter(circusPlaying.getFileCSVCircus(), true)) {
                        circusPlaying.getCsvCircus().writeLine(writer,
                                new String[]{circusPlaying.getUsername(),
                                        Integer.toString(CircusGame.getScore()),
